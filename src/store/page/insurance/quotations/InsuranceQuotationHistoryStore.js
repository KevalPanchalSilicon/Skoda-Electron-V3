...
export default class InsuranceQuotationHistoryStore {
	AUTH = null
	agGrid = null;
		...
		return Axios.post(`/insurance/quotations/pending_discount_list`, payload).then(({ data }) => {
			if (data.list.data.length) {
				data.list.data.map((item, index) => {
					item.date_changed = item.modified ? moment(item.modified).format("DD/MM/YYYY") : "N/A";
					return null;
				});
			}
...
decorate(InsuranceQuotationHistoryStore, {
	per_page: observable,
	agGrid: observable,
