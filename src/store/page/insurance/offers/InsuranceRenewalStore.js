...
export default class InsuranceRenewalStore {
	AUTH = null
	agGrid = null;
	...
	changeFilterAndSort = (params) => {
		...
		if (final_filter["status"]) {
			...
			final_filter["status"].values.forEach(x => (
				values_changed.push(globalStatus('insurance_renewals_type', 'value', x))
			))
	...
	getList = (payload) => {
		return Axios.post(`/insurance/offers/renewal_rollover_list`, payload).then(({ data }) => {
...
decorate(InsuranceRenewalStore, {
	per_page: observable,
	agGrid: observable,
