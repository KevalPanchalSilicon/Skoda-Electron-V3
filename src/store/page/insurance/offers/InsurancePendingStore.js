...
export default class InsurancePendingStore {
	AUTH = null
	agGrid = null;
	...
	setupGrid = (params) => {
		...
		let statusFilter;
		if (localStorage.getItem("activeInsurance")) {
			statusFilter = JSON.parse(localStorage.getItem("activeInsurance"));
	...
	changeFilterAndSort = (params) => {
		...
		if (final_filter["type_id"]) {
			...
			final_filter["type_id"].values.forEach(x => (
				values_changed.push(globalStatus('insurance_type', 'value', x))
			))
		...
		if (final_filter["status"]) {
			...
			final_filter["status"].values.forEach(x => (
				values_changed.push(globalStatus('insurance_status', 'value', x))
			))
	...
	getList = (payload) => {
		return Axios.post(`/insurance/offers/pending_list`, payload).then(({ data }) => {
			if (data.list.data.length) {
				data.list.data.map((item, index) => {
					item.nf_date = item.nf_date ? moment(item.nf_date).format("DD/MM/YYYY") : "N/A";
					return null;
				});
			}
			this.list_data =
...
decorate(InsurancePendingStore, {
	per_page: observable,
	agGrid: observable,
