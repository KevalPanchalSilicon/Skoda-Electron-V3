...
export default class ReceivedInsurancePayoutStore {
	agGrid = null;
	...
	getList = (payload) => {
		return Axios.post(`/insurance/payouts/received_list`, payload).then(({ data }) => {
			if (data.list.data.length) {
				data.list.data.map((item, index) => {
					item.ins_date = item.ins_date ? moment(item.ins_date).format("DD/MM/YYYY") : "N/A";
					item.ins_payout.date_received = item.ins_payout.date_received ? moment(item.ins_payout.date_received).format("DD/MM/YYYY") : "N/A";
...
decorate(ReceivedInsurancePayoutStore, {
	per_page: observable,
	agGrid: observable,
