...
export default class RejectedInsurancePayoutStore {
	agGrid = null;
	...
	getList = (payload) => {
		return Axios.post(`/insurance/payouts/rejected_list`, payload).then(({ data }) => {
			if (data.list.data.length) {
				data.list.data.map((item, index) => {
					item.ins_date = item.ins_date ? moment(item.ins_date).format("DD/MM/YYYY") : "N/A";
					item.ins_payout.date_rejected = item.ins_payout.date_rejected ? moment(item.ins_payout.date_rejected).format("DD/MM/YYYY") : "N/A";
					item.ins_payout.date_approval = item.ins_payout.date_approval ? moment(item.ins_payout.date_approval).format("DD/MM/YYYY") : "N/A";
					if (!item.booking) {
						let booking_object = {
							"co_no": "N/A",
							"inv_date": "N/A"
...
decorate(RejectedInsurancePayoutStore, {
	per_page: observable,
	agGrid: observable,
