...
export default class ChassisAllocationHistoryStore {
	AUTH = null
	agGrid = null;
	per_page = ServerGridConfig.options.paginationPageSize;
	current_page = 1;
	list_data = null;
	viewAllocatedValues = null;
	removeChassisValues = null;
	total = 0;
	allColumnIds = [];
	// type = {
	// 	pending: "Pending",
	...
	getList = (payload) => {
		return Axios.post(`/sales/chassis/history_list`, payload).then(({ data }) => {
	...
	// Call stock api
	RemoveChassisAllocationData = (formdata) => {
		return Axios.post(`/sales/chassis/destroy/` + formdata.id, formdata)
			.then(({ data }) => {
				this.setupGrid(this.agGrid);
				return data;
			})
...
decorate(ChassisAllocationHistoryStore, {
	per_page: observable,
	agGrid: observable,
	...
	viewAllocatedValues: observable,
	removeChassisValues: observable,
	type: observable,
	defaultType: observable,
	RemoveChassisAllocationData: action
});
