...
export default class DeliveredStore {
	AUTH = null
	agGrid = null;
	...
	setupGrid = (params) => {
		...
		let dateFilter;
		if (localStorage.getItem("deliveredDate")) {
			dateFilter = JSON.parse(localStorage.getItem("deliveredDate"));
	...
	getList = (payload) => {
		...
		return Axios.post(`/sales/manage_bookings/list_delivered`, payload).then(({ data }) => {
