...
export default class RejectedCorporatePayoutStore {
	agGrid = null;
	...
	getList = (payload) => {
		return Axios.post(`/corporate/payouts/rejected_list`, payload).then(({ data }) => {
			if (data.list.data.length) {
				data.list.data.map((item, index) => {
					item.inv_date = item.inv_date ? moment(item.inv_date).format("DD/MM/YYYY") : "N/A";
					item.corporate_payout.date_rejected = item.corporate_payout.date_rejected ? moment(item.corporate_payout.date_rejected).format("DD/MM/YYYY") : "N/A";
					item.corporate_payout.date_approval = item.corporate_payout.date_approval ? moment(item.corporate_payout.date_approval).format("DD/MM/YYYY") : "N/A";
					return null;
				});
			}
			this.list_data =
...
decorate(RejectedCorporatePayoutStore, {
	per_page: observable,
	agGrid: observable,
