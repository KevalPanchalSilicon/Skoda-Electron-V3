...
export default class ReceivedCorporatePayoutStore {
	agGrid = null;
	...
	getList = (payload) => {
		return Axios.post(`/corporate/payouts/received_list`, payload).then(({ data }) => {
			if (data.list.data.length) {
				data.list.data.map((item, index) => {
					item.inv_date = item.inv_date ? moment(item.inv_date).format("DD/MM/YYYY") : "N/A";
					item.corporate_payout.date_received = item.corporate_payout.date_received ? moment(item.corporate_payout.date_received).format("DD/MM/YYYY") : "N/A";
...
decorate(ReceivedCorporatePayoutStore, {
	per_page: observable,
	agGrid: observable,
