...
export default class ApprovedCorporatePayoutStore {
	agGrid = null;
	...
	getList = (payload) => {
		return Axios.post(`/corporate/payouts/approved_list`, payload).then(({ data }) => {
			if (data.list.data.length) {
				data.list.data.map((item, index) => {
					item.inv_date = item.inv_date ? moment(item.inv_date).format("DD/MM/YYYY") : "N/A";
					item.corporate_payout.date_claimed = item.corporate_payout.date_claimed ? moment(item.corporate_payout.date_claimed).format("DD/MM/YYYY") : "N/A";
...
decorate(ApprovedCorporatePayoutStore, {
	per_page: observable,
	agGrid: observable,
