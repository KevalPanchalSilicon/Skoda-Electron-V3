...
export default class CorporatePayoutsAllStore {
	agGrid = null;
	...
	payoutDetail = (formData) => {
		return Axios.post(`/corporate/payouts/detail/${formData.booking_id}`, formData)
			.then(({ data }) => {
	...
	changeFilterAndSort = (params) => {
		...
		if (final_filter["corporate_payout.status"]) {
			let values_changed = []
			final_filter["corporate_payout.status"].values.forEach(x => (
				values_changed.push(globalStatus('corporate_payout_status', 'value', x))
			))
			final_filter["corporate_payout.status"].values = values_changed
		}
	...
	getList = (payload) => {
		return Axios.post(`/corporate/payouts/all_list`, payload).then(({ data }) => {
	...
	RevertData = (id, formdata) => {
		return Axios.post(`/corporate/payouts/revert/${id}`, formdata).then(({ data }) => {
			return data
		}).catch(({ response: { data } }) => {
			var errors = [];
			var notify = null;
	...
	MarkAsReceivedData = (id, formdata) => {
		return Axios.post(`/corporate/payouts/mark_as_received/${id}`, formdata).then(({ data }) => {
			return data
		}).catch(({ response: { data } }) => {
			var errors = [];
			var notify = null;
	...
	MarkAsRejectedData = (id, formdata) => {
		return Axios.post(`/corporate/payouts/mark_as_rejected/${id}`, formdata).then(({ data }) => {
			return data
		}).catch(({ response: { data } }) => {
			var errors = [];
			var notify = null;
	...
	MarkAsApprovedData = (id, formdata) => {
		return Axios.post(`/corporate/payouts/mark_as_approved/${id}`, formdata).then(({ data }) => {
			return data
		}).catch(({ response: { data } }) => {
			var errors = [];
			var notify = null;
	...
	MarkAsClaimedData = (id, formdata) => {
		return Axios.post(`/corporate/payouts/mark_as_claimed/${id}`, formdata).then(({ data }) => {
			return data
		}).catch(({ response: { data } }) => {
			var errors = [];
			var notify = null;
...
decorate(CorporatePayoutsAllStore, {
	per_page: observable,
	agGrid: observable,
