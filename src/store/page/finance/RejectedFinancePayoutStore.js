...
export default class RejectedFinancePayoutStore {
	agGrid = null;
	...
	getList = (payload) => {
		return Axios.post(`/finance/payouts/rejected_list`, payload).then(({ data }) => {
			if (data.list.data.length) {
				data.list.data.map((item, index) => {
					item.inv_date = item.inv_date ? moment(item.inv_date).format("DD/MM/YYYY") : "N/A";
					item.finance_payout.date_rejected = item.finance_payout.date_rejected ? moment(item.finance_payout.date_rejected).format("DD/MM/YYYY") : "N/A";
...
decorate(RejectedFinancePayoutStore, {
	per_page: observable,
	agGrid: observable,
