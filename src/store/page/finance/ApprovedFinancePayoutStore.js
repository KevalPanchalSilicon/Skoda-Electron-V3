...
export default class ApprovedFinancePayoutStore {
	agGrid = null;
	...
	getList = (payload) => {
		return Axios.post(`/finance/payouts/approved_list`, payload).then(({ data }) => {
			if (data.list.data.length) {
				data.list.data.map((item, index) => {
					item.inv_date = item.inv_date ? moment(item.inv_date).format("DD/MM/YYYY") : "N/A";
					item.finance_payout.date_claimed = item.finance_payout.date_claimed ? moment(item.finance_payout.date_claimed).format("DD/MM/YYYY") : "N/A";
...
decorate(ApprovedFinancePayoutStore, {
	per_page: observable,
	agGrid: observable,
