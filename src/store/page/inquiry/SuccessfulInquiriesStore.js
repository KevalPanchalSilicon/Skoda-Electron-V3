...
export default class SuccessfulInquiriesStore {
	agGrid = null;
	...
	getList = () => {
		let filter;
		if (localStorage.getItem("successfulInqDate")) {
			filter = JSON.parse(localStorage.getItem("successfulInqDate"));
		}
		this.list_data = null;
		return Axios.post(`/inquiries/manage/list_successful_closure`).then(({ data }) => {
			if (data && data.list.data.length) {
				data.list.data.map((item, index) => {
					...
					item.closed_date_changed = item.closure_date ? moment(item.closure_date).format("DD/MM/YYYY") : "N/A";
					return null;
				});
			}
...
decorate(SuccessfulInquiriesStore, {
	per_page: observable,
	agGrid: observable,
