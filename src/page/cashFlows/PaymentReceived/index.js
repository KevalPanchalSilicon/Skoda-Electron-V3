...
const PaymentsReceived = observer((props) => {
	const {
		PaymentStore: {
			setPageSize,
			per_page,
	...
	const openConfirmModal = async (data, model, type = "get") => {
		setComponentType({ model, type, storeType: "Payment Received" })
	...
	return (
		<>
			<PageHeader
				title={BreadcrumbConfig.PaymentReceived.title}
				className="tableAreaSec"
				extra={<BreadcrumbComponent items={BreadcrumbConfig.PaymentReceived.path} />}
			>
