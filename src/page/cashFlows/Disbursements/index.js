...
const Disbursements = observer((props) => {
	...
	const openConfirmModal = async (data, model, type = "get") => {
		setComponentType({ model, type, storeType: "Disbursement" })
	...
	return (
		<>
			<PageHeader
				title={BreadcrumbConfig.Disbursements.title}
				className="tableAreaSec"
				extra={<BreadcrumbComponent items={BreadcrumbConfig.Disbursements.path} />}
			>
				<div className="listCountNew">
					{/* {AUTH.checkPrivileges("#12#") && ( */}
					<Button key="1" onClick={() => openConfirmModal(null, 3, "add")}>
						New
					</Button>
					{/* )} */}
					<RecordPerPage
