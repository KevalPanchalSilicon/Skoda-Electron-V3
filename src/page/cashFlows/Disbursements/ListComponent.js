...
import { CurrencyFormat, DateComparator, cashflow_payment_status, booking_status, cashflow_payment_mode } from "../../../utils/GlobalFunction";
...
const ActionRenderer = observer((props) => {
	...
	const generatePaymentDocument = () => {
		...
		PaymentStore.generatePaymentDocumentData(formData).then((data) => {
			...
			if (DisbursementStore.agGrid) {
				setupGrid(DisbursementStore.agGrid);
			}
		})
	}
		...
		<div className="action-column actionBtnWrapper">
			{(AUTH.checkPrivileges("#8010#") || AUTH.checkPrivileges("#8160#") || AUTH.checkPrivileges("#8187#") || AUTH.checkPrivileges("#8195#") || AUTH.checkPrivileges("#8255#") || AUTH.checkPrivileges("#8310#") || AUTH.checkPrivileges("#8205#")) && (
				<Button
					...
					style={{ padding: 7 }}
					onClick={() => {
						openConfirmModal(props.data, 3, "view");
					}}
				>
					<FontAwesomeIcon icon={faEye} />
				</Button>
			)}
			{(AUTH.checkPrivileges("#8013#") && (props.data.type === 20 ? true : props.data.status_id === 4) && props.data.doc_id === null) && (
				<Button
					type="text"
					title={"Generate"}
