...
const ReqRevertAccessoryComponent = observer((props) => {
	...
	const handleSubmit = (data) => {
		setSaving(true);
		// data.booking_id = ManageZFormsStore.reqRevertAccessoryOfferValues.id
		data.booking_id = ManageZFormsStore.viewValues.id
		data.note = form.getFieldValue("note");
		ManageZFormsStore.ReqRevertAccessoryOffer(data)
			.then((data) => {
				close();
				vsmNotify.success({
	...
	return ManageZFormsStore.viewValues ? (
		...
		>
			<Form form={form} id="reqRevertFormAccessory" labelCol={{ span: 24 }} onFinish={handleSubmit}>
				{
					<Row gutter={30} align="middle">
						<Col span={24}>
							<div className="revert_package_sec">
								<ul>
									<li>MIS or Admin can revert the discount on Accessory Offer, if calculation mode is OFF.</li>
...
export default ReqRevertAccessoryComponent;
