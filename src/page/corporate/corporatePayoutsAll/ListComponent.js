...
import { CurrencyFormat, DateComparator, corporate_payout_status, corporate_payout_statusArr, corporate_payout_status_color } from "../../../utils/GlobalFunction";
...
const ListComponent = observer((props) => {
	const { CorporatePayoutsAllStore: { setupGrid } } = useStore();
	...
	const gridOptions = {
		columnDefs: [
			...
			{
				headerName: "Payout Recd.",
				field: "corporate_payout.received",
				filter: "agNumberColumnFilter",
				cellRendererFramework: function (params) {
					return <CurrencyFormat value={params.data.corporate_payout.received} />
				},
			},
			{
				headerName: "Status",
				field: "corporate_payout.status",
				filter: "agSetColumnFilter",
				cellRendererFramework: function (params) {
					return <p className={corporate_payout_status_color[params.data.corporate_payout.status]}>{corporate_payout_status[params.data.corporate_payout.status]}</p>
				},
				filterParams: {
					defaultToNothingSelected: true,
					buttons: ['apply', 'reset'],
					values: (params) => {
						params.success(corporate_payout_statusArr)
					}
				},
			},
			{
