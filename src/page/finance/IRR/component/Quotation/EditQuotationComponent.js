...
const EditQuotationComponent = observer((props) => {
	...
	const handleSubmit = (data) => {
		...
		data.id = IRRPendingListStore.editQuotationValues.id
		IRRPendingListStore.EditData(data)
			.then((data) => {
				close();
				vsmNotify.success({
	...
	useEffect(() => {
		if (props.visible && IRRPendingListStore.finance_irr_detail && IRRPendingListStore.editQuotationValues) {
			IRRPendingListStore.dropdown_bank_list = [IRRPendingListStore.editQuotationValues.bank]
			IRRPendingListStore.dropdown_DSA_list = IRRPendingListStore.editQuotationValues.dsa_id > 0 ? [IRRPendingListStore.editQuotationValues.dsa] : null
			form.setFieldsValue({
				bank_id: IRRPendingListStore.editQuotationValues.bank_id,
				bank_payout: IRRPendingListStore.editQuotationValues.bank_payout,
				ex_showroom: IRRPendingListStore.finance_irr_detail.booking_ledger.ex_showroom,
				bank_irr: IRRPendingListStore.editQuotationValues.irr_comp,
				customer_irr: IRRPendingListStore.editQuotationValues.irr_cust,
				loan_amount: IRRPendingListStore.editQuotationValues.loan_amount,
				tenure: IRRPendingListStore.editQuotationValues.tenure,
				emi: IRRPendingListStore.editQuotationValues.emi,
				no_adv_emis: IRRPendingListStore.editQuotationValues.no_adv_emis,
				advance_arrear: IRRPendingListStore.editQuotationValues.adv_arrear,
				outstanding_emi: IRRPendingListStore.editQuotationValues.os_emis,
				plpm: IRRPendingListStore.editQuotationValues.plpm,
				gross_funding: IRRPendingListStore.editQuotationValues.gross_funding,
				effective_funding: IRRPendingListStore.editQuotationValues.eff_funding,
				net_loan: IRRPendingListStore.editQuotationValues.net_loan,
				rev_load: IRRPendingListStore.editQuotationValues.rev_load,
				reverse_plowback: IRRPendingListStore.editQuotationValues.rev_plowback,
				gross_income: IRRPendingListStore.editQuotationValues.gross_income,
				tds: IRRPendingListStore.editQuotationValues.tds,
				service_tax: IRRPendingListStore.editQuotationValues.service_tax,
				net_income: IRRPendingListStore.editQuotationValues.net_income,
				dsa_id: IRRPendingListStore.editQuotationValues.dsa_id,
				dsa_comm: IRRPendingListStore.editQuotationValues.dsa_comm,
				remarks_fin_exe: IRRPendingListStore.editQuotationValues.remarks_fin_exe,
				remarks_fin_mgr: IRRPendingListStore.editQuotationValues.remarks_fin_mgr,
			})

			if (IRRPendingListStore.editQuotationValues.dsa_id > 0) {
				setIsDSASelected(false)
			}
		}
	}, [form, props, IRRPendingListStore, IRRPendingListStore.finance_irr_detail, IRRPendingListStore.editQuotationValues])
	...
	const close = () => {
		props.close();
		form.resetFields();
		IRRPendingListStore.editQuotationValues = null
		setSaving();
		setFetchBank(true);
		setFetchDSA(true);
		// setDisabled(true);
	...
	return IRRPendingListStore.finance_irr_detail && IRRPendingListStore.editQuotationValues ? (
		<Drawer
			className="addModal"
			title={"Edit Quotation (" + IRRPendingListStore.editQuotationValues.id + ")"}
			width="70%"
