...
import { vsmLogoUploadCS, vsmNotify } from "../../../../config/messages";
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import { faTimes } from "@fortawesome/free-solid-svg-icons";
import FormItem from "antd/lib/form/FormItem";
import { UploadIcon } from "../../../../config/IconsConfig";

const EditLogoImageComponent = observer((props) => {
	const [form] = Form.useForm();
	const { AUTH, CompanySettingStore: { company_data, EditLogoImageData }, } = useStore();
	...
	const handleSubmit = (data) => {
		...
		formData.append("logo_id", company_data.profile.logo_id)
		if (fileList.length > 0 && isImageUploaded) {
			formData.append('logo_file', fileList[0])
		}
		EditLogoImageData(formData, company_data.id)
			.then((data) => {
				close();
	...
	const onRemoveImage = () => {
		form.setFields([{ name: "logo_file", errors: [] }]);
		updateFileList([]);
				...
				<Row gutter={30}>
					<Col xs={{ span: 24 }}>
						<FormItem required labelCol={{ span: 24 }}>
							<Form.Item name="logo_file" rules={vsmLogoUploadCS.validation.logo_file}>
								<Upload
									accept=".png,.jpeg,.gif"
									fileList={fileList}
...
export default EditLogoImageComponent;
