...
import { vsmEmailBannerCS, vsmNotify } from "../../../../config/messages";
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import { faTimes } from "@fortawesome/free-solid-svg-icons";
import FormItem from "antd/lib/form/FormItem";
import { UploadIcon } from "../../../../config/IconsConfig";

const EditEmailHeaderComponent = observer((props) => {
	const [form] = Form.useForm();
	const { CompanySettingStore: { company_data, EditEmailHeaderData }, } = useStore();
	...
	const handleSubmit = (data) => {
		...
		formData.append("email_banner_id", company_data.email_banner_id)
		if (fileList.length > 0 && isImageUploaded) {
			formData.append('email_banner_file', fileList[0])
		}
		EditEmailHeaderData(formData, company_data.id)
			.then((data) => {
				close();
				vsmNotify.success({
	...
	const onRemoveImage = () => {
		form.setFields([{ name: "email_banner_file", errors: [] }]);
		updateFileList([]);
				...
				<Row gutter={30}>
					<Col xs={{ span: 24 }}>
						<FormItem required labelCol={{ span: 24 }}>
							<Form.Item name="email_banner_file" rules={vsmEmailBannerCS.validation.email_banner_file}>
								<Upload
									accept=".png,.jpeg,.gif"
									fileList={fileList}
...
export default EditEmailHeaderComponent;
