...
const ScopeComponent = observer((props) => {
	...
	const {
		ManageUserStore,
		ManageUserStore: { EditScopeData, scopeValues },
	} = useStore();
		...
		if (data.privileges) {
			data.privileges.map((menuItem) => {
				menuItem && menuItem.submenu.map((subItem) => {
					...
					// const subitem_length = subItem.actions ? subItem.actions.length : 0
	...
	useEffect(() => {
		if (scopeValues && props.visible) {
			const privileges = scopeValues.is_override === 1 ? scopeValues.privileges : scopeValues.role.privileges
			ManageUserStore.getPrivileges(privileges).then((data) => {
				form.setFieldsValue({
					privileges: data.list.data,
				});
			})
		}
	}, [ManageUserStore, scopeValues, form, props]);
											...
											return (
												<TabPane tab={value.name} key={value.id}>
													{value.submenu.map((subItem, subIndex) => {
														// var all_checked = 0;
														// var actions_length = subItem.actions ? subItem.actions.length : 0
														// subItem.actions && subItem.actions.map((action, index) => {
