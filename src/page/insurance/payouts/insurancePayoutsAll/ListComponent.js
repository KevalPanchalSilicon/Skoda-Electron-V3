...
import { CurrencyFormat, DateComparator, insurance_payout_status, finance_payout_statusArr, insurance_payout_status_color } from "../../../../utils/GlobalFunction";
...
const ListComponent = observer((props) => {
	const { InsurancePayoutsAllStore: { setupGrid } } = useStore();
	...
	const gridOptions = {
		columnDefs: [
			...
			{
				headerName: "Status",
				field: "ins_payout.status",
				filter: "agSetColumnFilter",
				cellRendererFramework: function (params) {
					return <p className={insurance_payout_status_color[params.data?.ins_payout?.status]}>{insurance_payout_status[params.data?.ins_payout?.status]}</p>
				},
				filterParams: {
					defaultToNothingSelected: true,
					buttons: ['apply', 'reset'],
					values: (params) => {
						params.success(finance_payout_statusArr)
					}
				},
			},
			{
