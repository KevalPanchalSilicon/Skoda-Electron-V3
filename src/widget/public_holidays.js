...
const PublicHolidays = observer((props) => {
	...
	useEffect(() => {
		if (props.api_url && !WidgetStore.list_public_holidays) {
			setLoading(true)
			WidgetStore.getListData(props.api_url, "list_public_holidays").then((data) => {
				setLoading(false);
			}).catch((e) => {
				setLoading(false)
			})
			...
			<div style={{ maxHeight: isOpen ? "999px" : "0" }} className="widget_wrapper">
				{
					loading ?
						<Skeleton active />
						:
						WidgetStore.list_public_holidays?.length > 0 ?
							<ul className="widget_data">
								{WidgetStore.list_public_holidays.map((item, index) => {
